  ...
  return (
    <>
      {isloading ? (
        ...
        <>
          <div className="p-2 sm:p-4 bg-white rounded-lg shadow h-full">
            {/* Header - responsive layout */}
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 space-y-2 sm:space-y-0">
              <h2 className="text-lg sm:text-xl font-bold text-gray-800">
                {monthTitle}
              </h2>
              <div className="text-xs sm:text-sm text-gray-600 flex items-center space-x-2 sm:space-x-4">
          ...
          <Modal
            ...
            <ModalContent>
              {() => (
                <>
                  <ModalHeader className="flex flex-col gap-2 border-b border-gray-200 pb-2 px-4 sm:px-6">
                    {selectedDay && (
                  ...
                  <ModalBody className="px-4 sm:px-6 pb-6">
                    {selectedDay && (
                      <div className="bg-white p-3 sm:p-6 rounded-xl shadow-sm border border-gray-200 space-y-4 sm:space-y-6">
                        <h3 className="text-base sm:text-lg font-semibold text-gray-900 border-b border-gray-100 pb-2">
                          Attendance Details
                        </h3>

                        {/* Responsive grid - single column on mobile */}
                        <div className="grid grid-cols-1 gap-y-3 sm:gap-y-4 text-sm text-gray-700">
...
const Legend = ({ color, label }) => (
  <span className="flex items-center space-x-1">
    <span
      className={`inline-block w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-${color}-100 border border-${color}-400`}></span>
