import React from "react";
import { Breadcrumbs, BreadcrumbItem } from "@nextui-org/react";

const BreadcrumbsComponent = ({ items }) => {
  const [currentPage, setCurrentPage] = React.useState("music");

  return (
    <div className="overflow-x-auto pb-1 w-full">
      <Breadcrumbs
        classNames={{
          list: "gap-1 sm:gap-2 flex-nowrap",
          base: "w-full",
        }}
        itemClasses={{
          item: [""],
          separator: "hidden",
        }}
        size="sm"
        onAction={(key) => setCurrentPage(key)}>
        {items?.map((item, index) => (
          <BreadcrumbItem
            key={index}
            href={item.href}
            isCurrent={currentPage === `{item.label}`}>
            <span className="relative inline-flex items-center h-6">
              <div className="relative w-20 sm:w-24 md:w-32 lg:w-40 xl:w-48 h-8 sm:h-10 bg-teal-800 hover:bg-red-600 pl-2 text-white text-xs text-center flex items-center justify-center clip-ribbon whitespace-nowrap overflow-hidden text-ellipsis">
                {item.label}
              </div>
            </span>
          </BreadcrumbItem>
        ))}
      </Breadcrumbs>
    </div>
  );
};

export default BreadcrumbsComponent;
